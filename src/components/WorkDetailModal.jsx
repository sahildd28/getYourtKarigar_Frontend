...
export default function WorkDetailModal({
  ...
  <div className="relative z-10 flex w-full max-w-[calc(100vw-1.5rem)] max-h-[92dvh] flex-col overflow-y-auto rounded-2xl border border-white/5 bg-[color:var(--bg-900)] shadow-2xl sm:max-w-4xl lg:flex-row lg:overflow-hidden">
        ...
        <div className="flex min-h-0 flex-col lg:max-h-[92dvh] lg:w-2/3">
          ...
          {images.length > 1 && (
            <div className="relative z-10 flex gap-2 overflow-x-auto border-t border-white/5 bg-black/70 px-4 py-3 lg:px-6">
              {images.map((img, idx) => (
                <button
                  key={img.id || idx}
                  type="button"
        ...
        <div className="flex min-h-0 flex-1 flex-col px-5 pb-5 pt-14 lg:w-1/3 lg:px-6 lg:pb-6 lg:pt-6">
